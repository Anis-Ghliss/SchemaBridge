...
export function OnboardingWizard() {
  const { dismissOnboarding, setView } = useAppStore();
      ...
      <div className="relative flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-xl border border-border bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-t border-border bg-muted/40 px-8 py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {step === 1 && (
              <Button variant="ghost" size="sm" onClick={close}>I'll explore on my own</Button>
            )}
          ...
          <div key={step.id} className="flex flex-1 items-center gap-2">
            <div
              className={cn(
                ...
                done ? "bg-primary text-white" : active ? "bg-primary/15 text-primary" : "bg-muted text-slate-400"
              )}
            >
            ...
            {step.id < STEPS.length && <div className={cn("h-px flex-1", done ? "bg-primary/30" : "bg-border")} />}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="space-y-5 py-6">
      <div className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-2.5 py-1 text-[11px] font-medium text-primary">
        <Sparkles className="h-3 w-3" /> Welcome to SchemaBridge
      </div>
      <h2 className="text-xl font-semibold">Two services, two shapes, one bridge.</h2>
      <p className="text-sm text-slate-600">
        Picture this: <strong>Service A</strong> sends requests in its current shape and <strong>Service B</strong> just changed what it expects. Instead of editing Service A (maybe you don't own it) or writing adapter code in both places, you point Service A at SchemaBridge — and the bridge translates each request in flight before forwarding it to Service B.
      </p>

      <div className="grid gap-3 lg:grid-cols-3">
        <Card icon={Layers3} title="1. Capture both shapes" body="Paste the old and new payloads — that's how the bridge learns what to translate." />
        <Card icon={GitBranch} title="2. Connect the fields" body="Drag (or accept the suggestions) so each field in the old shape maps to its place in the new shape." />
        <Card icon={Send} title="3. Send traffic through" body="Point Service A at the bridge port. We'll reshape every request and forward it for you." />
      </div>

      <p className="text-xs text-slate-500">
        This will take about a minute. You can <button type="button" className="underline hover:text-foreground" onClick={onSkip}>skip and explore on your own</button> at any time.
      </p>
    </div>
  );
}

function Card({ icon: Icon, title, body }: { readonly icon: typeof Layers3; readonly title: string; readonly body: string }) {
  return (
...
function ConnectStep({ sourceFields, targetFields, rules, onChange }: ConnectStepProps) {
  ...
  function autoSuggest() {
    onChange(suggestMappings(sourceFields, targetFields));
  }
    ...
    <div className="space-y-4 py-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="secondary" size="sm" onClick={autoSuggest}><Sparkles className="h-3.5 w-3.5" /> Re-suggest</Button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-lg border border-border">
        ...
        {targetLeaves.length === 0 ? (
          <p className="px-4 py-6 text-center text-xs text-slate-500">The target schema has no leaf fields yet — go back and adjust the payload.</p>
        ) : (
  ...
  return (
    <div className="space-y-4 py-6">
      ...
      <div className="rounded-lg border border-border bg-muted/40 p-4 text-xs text-slate-600">
        <strong>Next:</strong> after this step, point Service A at <code className="rounded bg-white px-1 py-0.5">http://&lt;bridge-host&gt;:8080{draft.pathPattern}</code> and SchemaBridge will translate every request before forwarding to <code className="rounded bg-white px-1 py-0.5">{draft.upstreamBaseUrl}{draft.pathPattern}</code>. You'll land in the <strong>Live</strong> view so you can watch the traffic come through.
      </div>
    </div>
  );
}
